...
//  },[null])
  ...
  const handleSubmit = async (event) => {
    ...
    await axios.post("https://truth-glib-star.glitch.me/forgetPassword",data).then((responce)=>{localStorage.setItem("otp",responce.data.message);localStorage.setItem("email",email)  ; setOtp(responce.data.message);navigate('/otpverify');}).catch((e)=>{if(e.response.data.message=="no email found") {setEmailError(e.response.data.message)} })
  }
